...
class Predict:
    def __init__(self, model_dir, model_name):
        ...
        with tf.Graph().as_default() as graph:
            tf.import_graph_def(graph_def, name='softmax')

        self.graph = graph

        self.X = graph.get_tensor_by_name('softmax/input/X:0')
        self.predict = graph.get_tensor_by_name('softmax/output/predict:0')

    def __call__(self, X):
        input = self.graph.get_tensor_by_name('softmax/input/X:0')
        predict = self.graph.get_tensor_by_name('softmax/output/predict:0')
